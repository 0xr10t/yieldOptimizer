module vault_addr::lending_protocol {
    ...
    struct Pool has key {
        ...
        // Store mint ref for underlying asset
        underlying_mint_ref: MintRef,
    }
    ...
    public fun deposit(pool_addr: address, asset_to_supply: FungibleAsset): FungibleAsset acquires Pool {
        ...
        // In a real protocol, `asset_to_supply` would be stored. Here we just burn it using the underlying burn ref.
        fungible_asset::burn(&pool.receipt_token_burn_ref, asset_to_supply);
    ...
    public fun withdraw(pool_addr: address, receipt_asset: FungibleAsset, _metadata: Object<Metadata>): FungibleAsset acquires Pool {
        let pool = borrow_global_mut<Pool>(pool_addr);
        let amount = fungible_asset::amount(&receipt_asset);
        fungible_asset::burn(&pool.receipt_token_burn_ref, receipt_asset);
        pool.total_supplied = pool.total_supplied - (amount as u128);
        // Mint the underlying asset back to the user using stored mint ref
        fungible_asset::mint(&pool.underlying_mint_ref, amount)
    }
    ...
    public fun initialize_mock_protocol(deployer: &signer) {
        ...
        let underlying_constructor = &object::create_named_object(deployer, b"UnderlyingToken");
        primary_fungible_store::create_primary_store_enabled_fungible_asset(
            underlying_constructor, option::none(), string::utf8(b"Mock USDC"), string::utf8(b"mUSDC"), 6, string::utf8(b""), string::utf8(b"")
        );
        move_to(deployer, Pool {
            ...
            underlying_mint_ref: fungible_asset::generate_mint_ref(underlying_constructor),
        });
    }
}